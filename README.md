...
<head>
    ...
    <title>배달 경로 최적화 시뮬레이션 (TSP Brute Force)</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <!-- Header -->
        <div class="header">
            <h1>🚚 배달 경로 최적화 시뮬레이션</h1>
            <p>물류센터(Depot)에서 출발하여 모든 고객 주소를 방문한 후<br>
            가장 짧은 거리로 다시 물류센터로 돌아오는 최적 경로를 찾습니다.</p>
        </div>
        ...
        <div class="main-content">
            ...
            <div class="control-panel">
                ...
                <div class="form-group">
                    <label>지점 수 (물류센터 포함)</label>
                    <input type="number" id="numPoints" min="2" max="10" value="5" placeholder="2-10 권장">
                </div>
                
                <button class="btn btn-primary" onclick="generateRandomPoints()">🎲 랜덤 지점 생성</button>
                
                <div class="form-group">
                    <label>새 지점 좌표 (x, y)</label>
                    <input type="text" id="newPointX" placeholder="X 좌표 (예: 10)">
                    <input type="text" id="newPointY" placeholder="Y 좌표 (예: 20)" style="margin-top: 10px;">
                </div>
                
                <button class="btn btn-primary" onclick="addPoint()">➕ 지점 추가</button>
                
                <div class="points-list" id="pointsList">
                    <p style="text-align: center; color: #999;">지점을 추가해주세요</p>
                </div>
                
                <button class="btn btn-success" id="solveBtn" onclick="solveTSP()" disabled>🎯 최적 경로 찾기</button>
            ...
            <div class="visualization-panel">
                ...
                <div class="controls-row">
                    <button class="btn btn-primary" onclick="toggleAllEdges()" title="모든 가능한 경로 선 표시/숨김">🔀 전체 엣지 토글</button>
                    <button class="btn btn-primary" onclick="animatePath()" title="최적 경로를 단계별로 그립니다 (먼저 경로를 찾아야 합니다)">🎬 경로 애니메이션</button>
                </div>
            </div>
        </div>
        ...
        <div class="results-panel" id="resultsPanel">
            <h2 class="result-header">✅ 최적 경로 탐색 완료!</h2>
            
            <div class="result-stats">
                <div class="stat-card">
                    <div class="stat-label">최소 총 이동 거리</div>
                    <div class="stat-value" id="minDistance">0</div>
                    <div class="stat-label">거리 단위</div>
                </div>
                
                <div class="stat-card">
                    <div class="stat-label">탐색한 경로 수</div>
                    <div class="stat-value" id="pathsExplored">0</div>
                    <div class="stat-label">개</div>
                </div>
                
                <div class="stat-card">
                    <div class="stat-label">방문 지점 수</div>
            ...
            <div class="path-display">
                ...
                <p style="color: #666; margin-top: 10px; font-size: 0.9em;">
                    (P0: 물류센터/Depot, P1~: 고객 주소/Customer Addresses)
                </p>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function updatePointsList() {
            ...
            if (Object.keys(points).length === 0) {
                listDiv.innerHTML = '<p style="text-align: center; color: #999;">지점을 추가해주세요</p>';
                return;
            }
        ...
        function solveTSP() {
            ...
            loadingDiv.innerHTML = '🔄 최적 경로 탐색 중...';
        ...
        function showResults(pathsExplored) {
            ...
            resultsPanel.innerHTML = `
                ...
                <div class="path-display">
                    <h3>🚚 최적 배달 순서</h3>
                    <div class="path-sequence">${bestPath.map(i => `P${i}`).join(' → ')} → P${bestPath[0]}</div>
                    <p style="color: #666; margin-top: 10px; font-size: 0.9em;">
                        (P0: 물류센터/Depot, P1~: 고객 주소/Customer Addresses)
                    </p>
                </div>
        ...
        function drawCanvas(progress = 1) {
            ...
            if (Object.keys(points).length === 0) {
                ...
                ctx.fillText('지점을 추가하거나 랜덤 생성해주세요', canvas.width / 2, canvas.height / 2);
                return;
            }
            ...
            if (progress < 1 && bestPath.length > 0) {
                ...
                ctx.font = 'bold 14px Arial';
                ctx.textAlign = 'left';
                ctx.fillText(`애니메이션 진행: ${Math.floor(progress * 100)}%`, 20, 35);
            }
        }
        ...
        function animatePath() {
            if (bestPath.length === 0) {
                alert('❌ 먼저 "최적 경로 찾기" 버튼을 눌러 경로를 계산해주세요!');
                return;
            }
            
            if (animating) {
                alert('⏸️ 애니메이션이 이미 실행 중입니다.');
                return;
            }
            ...
            notification.style.cssText = `
                ...
                font-weight: bold;
            `;
            notification.textContent = '🎬 경로 애니메이션 시작!';
            document.body.appendChild(notification);
            
            setTimeout(() => notification.remove(), 2000);
            
            function animate() {
                animationProgress += 0.015; // 조금 더 천천히
                if (animationProgress >= 1) {
                    ...
                    doneNotif.style.cssText = `
                        ...
                        font-weight: bold;
                    `;
                    doneNotif.textContent = '✅ 애니메이션 완료!';
